...
public class PartTwo {
    ...
    private static int calculateReflectionValue(List<String> binaryList){
        for (int i = 0; i < binaryList.size() - 1; i++){
            ...
            while (left > 0 && right < binaryList.size() - 1){
                if (smudgeMatch(binaryList.get(left), binaryList.get(right))){
                    smudgeCount++;
                } else if ((!reflects(binaryList.get(left), binaryList.get(right))) || smudgeCount > 1){
                    break;
                }
            ...
            if ((left == 0 || right == binaryList.size() - 1) && smudgeCount == 1 && ((reflects(binaryList.get(left), binaryList.get(right)))) || smudgeMatch(binaryList.get(left), binaryList.get(right))){
                return i + 1;
            }
        }
    ...
    private static boolean smudgeMatch(String first, String second){
        int binaryValue = (Integer.parseInt(first, 2) ^ (Integer.parseInt(second, 2)));
        
        
        String test = Integer.toBinaryString(binaryValue);
